...
for m in hsb_models:
    MODEL_DIR = f"/scratch/jliebl/ASR_ComputEL/ASR_ComputEL/Models/{m}"
    ...
    eval_df.to_csv(f"/scratch/jliebl/ASR_ComputEL/ASR_ComputEL/Results/{m}_eval_results_hsb.tsv", sep="\t", index=False)
...
for m in lg_models:
    ...
    eval_df["Prediction"] = preds
    eval_df["CER scores"] = cers
    eval_df.to_csv(f"/scratch/jliebl/ASR_ComputEL/ASR_ComputEL/Results/{m}_eval_results_lg.tsv", sep="\t", index=False)
...
for m in tt_models:
    ...
    eval_df["Prediction"] = preds
    eval_df["CER scores"] = cers
    eval_df.to_csv(f"/scratch/jliebl/ASR_ComputEL/ASR_ComputEL/Results/{m}_eval_results_tt.tsv", sep="\t", index=False)
