...
for t in train:
    ...
    samp_df = full_df.sample(n=2800, random_state=seed)
    train_df = samp_df.iloc[:2000, :]
    test_df = samp_df.iloc[2000:2400, :]
    eval_df = samp_df.iloc[2400:, :]
    ...
    eval_df.to_csv(f"/scratch/jliebl/ASR_ComputEL/CV_VC_Data/Dataframes/eval/{t}_eval.tsv", sep="\t", index=False)
